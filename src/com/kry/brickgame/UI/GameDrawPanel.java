	...
	public void paintComponent(Graphics g) {
		...
		if ((null == canvas) || (canvas.getWidth() != size.width)
			...
			if (backgroundImage != null) {
				g2d.drawImage(backgroundImage, 0, 0, size.width, size.height, deviceBgColor, this);
			} else {
				g2d.setBackground(deviceBgColor);
				g2d.clearRect(0, 0, size.width, size.height);
			}
		...
		BufferedImage gameField = getDrawer().getDrawnGameField(gameFieldWidth + 4,
				gameFieldHeight + 4, properties);

		/* Canvas graphics */
		g2d = (Graphics2D) canvas.getGraphics();

		g2d.drawImage(gameField, outBorderSpace + inBorderHorSpace - 2, outBorderSpace
				+ inBorderVertSpace - 2, gameField.getWidth(), gameField.getHeight(), this);
